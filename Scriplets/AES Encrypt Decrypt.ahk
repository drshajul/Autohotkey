...
File_AES(sFileFr, sFileTo, sPassword, SID = 256, bEncrypt = True)
{
	...
	nSize := File_GetFileSize(hFileFr),	VarSetCapacity(sData, nSize + (bEncrypt ? 16 : 0))
	File_ReadFile(hFileFr, &sData, nSize),	File_CloseHandle(hFileFr)
	...
	File_WriteFile(hFileTo, &sData, nSize),	File_CloseHandle(hFileTo)
	Return	nSize
}
