...
Loop,3  ;Loop through IFD's, usually only two
{
  If nextIFD  ;If next IFD exists
    {
    rData := 1   ;Reset
    MsgBox % "Going to IFD" . A_Index-1 . " at : " . nextIFD
    oFile.Seek(nextIFD)
  ...
  If subIFDOffset
    {
    rData := 1
    MsgBox % "Going to subIFD at : " . subIFDOffset
  ...
  If InterOpOffset
    {
    rData := 1
    MsgBox % "Going to InterOp at : " . InterOpOffset
...
Loop, % thisIFDnEntries
{
  ...
  s .= "`n" . thisTag . " : " . oTag[thisTag]  ;Description of Tag
  ...
  s .= "`n" . "Type is : " . thisType
  thisSize := oType[thisType].len
  s .= "`n" . "Size of component is : " . thisSize
  ...
  s .= "`n" . "Total Data length is : " . thisDataLen . "bytes"
  ...
  if (thisTag = "8769") ;This is offset to SubIFD
  {
     ...
     s .= "`n" . "It is subIFD offset at : " . subIFDOffset
  }
  else if (thisTag = "A005") ;This is offset to InterOpIFD
  {
     ...
     s .= "`n" . "It is InterOpIFD offset at : " . InterOpOffset
  }
  else if (thisDataLen > 4) ;The value is an offset
  {
     ...
     s .= "`n" . "It is an offset at : " . corrOffset
     lastpos := oFile.position
     oFile.Seek(corrOffset)
     oFile.RawRead(rData,thisDataLen)
     if (thisType = 2) ;It is an ascii string
        s .= "`n" . "Data is ascii string : " . StrGet(&rData,thisDataLen,"CP0") ;read System ANSI codepage string
     else if (thisType=5 || thisType=10) ;It is an rational
        {
        nNum := NumGet(rData,0,"UInt")
        nDen := NumGet(rData,4,"UInt")
        s .= "`n" . "Data is : " . nNum . "/" . nDen
        }
     else
        s .= "`n" . "Data is : " . NumGet(rData,0,oType[thisType].type)
     oFile.Seek(lastpos)
  }
